...
module attributes {torch.debug_module_name = "Net"} {
  ...
  func.func @matmul() -> tensor<256x4x37x14x96xf32> {
    ...
    %11 = linalg.generic {indexing_maps = [#map, #map1, #map], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel"], library_call = "none"} ins(%7, %reduced : tensor<256x7x74x27x96xf32>, tensor<256x7x74x27xf32>) outs(%8 : tensor<256x7x74x27x96xf32>) attrs =  {tag = "operation_6"} {
    ^bb0(%in: f32, %in_1: f32, %out: f32):
      %17 = arith.subf %in, %in_1 : f32
      %18 = math.exp %17 : f32
      linalg.yield %18 : f32
    } -> tensor<256x7x74x27x96xf32>
    %12 = bufferization.alloc_tensor() : tensor<1x1x1xf32>
    %13 = bufferization.alloc_tensor() : tensor<256x4x37x14x96xf32>
    %14 = linalg.pooling_ndhwc_max {dilations = dense<1> : tensor<3xi64>, strides = dense<2> : tensor<3xi64>, tag = "operation_7"} ins(%11, %12 : tensor<256x7x74x27x96xf32>, tensor<1x1x1xf32>) outs(%13 : tensor<256x4x37x14x96xf32>) -> tensor<256x4x37x14x96xf32>
